...
"""
脚本名称：12_run_dataset_build_v3_pipeline.py

功能：
1) 读取 dry-run / export / balanced 三份 YAML 参数；
2) 顺序执行 scripts/data_prep/10、scripts/data_prep/11、scripts/data_prep/07；
3) 统一输出每一步命令，便于复现与排错。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run dataset v3 (11block-only 8-class) build pipeline with YAML configs.")
    parser.add_argument("--dry-run-config", default=str(DEFAULT_DRYRUN_CFG), help="10 dry-run 的 YAML 参数文件")
    parser.add_argument("--export-config", default=str(DEFAULT_EXPORT_CFG), help="11 export 的 YAML 参数文件")
...
def main() -> None:
    ...
    if not args.skip_dry_run:
        run_stage(
            ...
            stage_name="dry-run (11block-only)",
        )
    if not args.skip_export:
        run_stage(
            ...
            stage_name="export (11block-only)",
        )
    ...
    print("\n[OK] dataset v3 (11block-only 8-class) pipeline completed.")
