...
"""
脚本名称：11_export_vit_dataset_tiles_11block_only.py

功能：
1) 读取 11block-only dry-run 生成的 tiles_manifest.csv；
2) 按 split 将影像/标签窗口导出为实体 patch；
3) 输出 export_report.csv，并可复制 dry-run 附属清单。
"""
...
DEFAULT_TILES_MANIFEST: Final[Path] = (
    PROJECT_ROOT / "data" / "processed" / "vit_dataset" / "v3_11block8" / "manifests" / "tiles_manifest.csv"
)
DEFAULT_MANIFESTS_DIR: Final[Path] = (
    PROJECT_ROOT / "data" / "processed" / "vit_dataset" / "v3_11block8" / "manifests"
)
DEFAULT_OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "data" / "processed" / "vit_dataset" / "v3_11block8"
...
VALID_SPLITS: Final[set[str]] = {"train", "val", "test"}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="按 11block-only tiles_manifest 导出 ViT 数据集 patch")
    ...
    parser.add_argument("--manifests-dir", default=str(DEFAULT_MANIFESTS_DIR), help="dry-run manifests 目录（用于拷贝附属清单）")
...
def main() -> None:
    ...
    if not pair.imagery_path.exists() or not pair.label_path.exists():
        raise FileNotFoundError(f"11block 输入不存在：{pair.imagery_path} / {pair.label_path}")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if pair_name != "11block":
            counters["failed"] += 1
            log(f"[WARN] pair_name 非 11block，已跳过: {pair_name} (tile_id={tile_id})")
            maybe_report(idx)
            continue
